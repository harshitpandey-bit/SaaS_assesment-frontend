...
const AdminLogin = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await axios.post('https://saas-assesment-backend.onrender.com/api/admin/login', {
        username,
        password,
      });

      if (response.data.token) {
        // Store token and set user role to "admin"
        localStorage.setItem('token', response.data.token);
        setUserRole('admin'); // Update userRole in context
